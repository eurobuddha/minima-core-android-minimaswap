...
import com.eurobuddha.minimaswap.eth.EthHtlc;
import com.eurobuddha.minimaswap.eth.EthNet;
...
public final class SwapEngine {
    ...
    public void startMinimaToErc20(Order maker, String sellMinima, String tokenSymbol, String buyTokenAmount, boolean otc, StartCb cb) {
        ...
        // M3: a responder burst-lock (or a coin-split) draws MINIMA from the SAME coin pool that this
    ...
    private void checkEthContractFor(EthHtlc eth, SwapDb.Swap s, String myEth) throws Exception {
        ...
        if (iAmReceiver) {
            ...
            // marked COMPLETE while the tokens are still in the vault.
    ...
    // possible: the ETH wallet is shared byte-for-byte with usdtSwap, so nonces can collide) then looked
    // "done", was skipped by allSwaps(), and never retried — stranding funds with no in-app recovery. Now
    ...
    /** Broadcast a withdraw (reveals the preimage, claims the tokens). Never finalizes — {@code gc.withdrawn}
    ...
    private void ensureAllowanceBlocking(EthHtlc eth, String token, BigInteger needed) throws Exception {
        ...
        // allowance — e.g. left by usdtSwap on the shared seed-derived wallet — must be zeroed first, or EVERY
